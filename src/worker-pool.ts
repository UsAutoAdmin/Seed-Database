...
import { writeSoldCount, flagForReview, SeenUrlTracker, isBrokenUrl } from "./db.js";
...
export class WorkerPool extends EventEmitter {
  ...
  loadTasks(tasks: ScrapeTask[]): void {
    this.queue = [...tasks];
  ...
  private async runWorker(workerId: number): Promise<void> {
    ...
    try {
      while (!this._stopping) {
        ...
        this.stats.queueRemaining = this.queue.length;
        await this.processTask(page, task, workerId);
  ...
  private async processTask(
    ...
    // Skip broken URLs (e.g. unescaped & in model names)
    if (isBrokenUrl(task.sold_link)) {
      this.stats.flagged++;
      this.stats.queueRemaining = this.queue.length;
      await flagForReview([task.id]);
      this.emit("task:flagged", {
        workerId,
        taskId: task.id,
        url: task.sold_link,
      });
    ...
    // Cross-batch duplicate detection (compares _nkw search query, not full URL)
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      ...
      if (error && isCaptchaOrBlock(error)) {
        ...
        await new Promise((r) =>
          setTimeout(r, 10_000 + Math.random() * 10_000)
        );
      }
